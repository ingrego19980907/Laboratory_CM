...
import pandas as pd #Щоб не використовувати довгу назву pandas, скорочуємо подальше посилання на бібліотеку до двох літер
import math
...
xlsx=pd.ExcelFile(r'G:\GEXF\МКДС/3.xlsx') #вводиться змінна xlsx, яка має значення рядка, що є фізичною адресою файла Excel
...
df=pd.read_excel(xlsx,usecols='A,B',header=None,names=namescol,nrows=50,converters={'PE':stint1,'Diameter':stint})
print(df)
...
sorted_df.to_excel('G:/GEXF/МКДС/3n.xlsx',header=False, index=False)

df['CC']=df['PE']+df['Diameter']
df['FFFF']=10.*df['CC']-df['Diameter']
print(df)
...
#Зовнішній вимір буде відповідати кількості груп, а внутрішній - дорівнює двом, тобто відповідає двом різним значенням у першому стовпчику
sss=[]
for i in range(ngroup):
    sss.append([])
    for j in range(2):
    ...
    for j in range(ngroup):
            if df['Diameter'][i]==kl[j]:    #Вибираємо елемент j-ої групи
                if df['PE'][i]==0.0:        #Якщо стовпчику PE для цільового атрибута стоїть значення нуль, то
                    sss[j][0]=sss[j][0]+1.  #рахуємо нулі і результат заносимо на першу позицію j-ої групи
                else:
                    sss[j][1]=sss[j][1]+1.  #У іншому випадку рахуємо одиниці і результат заносимо на другу позицію j-ої групи
print(sss)
...
for i in range(ngroup):
    EG=0.
    if sss[i][0]!=0.:
        EG=EG-sss[i][0]/nig[i]*math.log(sss[i][0]/nig[i]) #Якщо кількість елементів в і-ій групі з нульовими значеннями в першому стовпчику дорівнює нулю, то дія не виконується
    if sss[i][1]!=0.:
        EG=EG-sss[i][1]/nig[i]*math.log(sss[i][1]/nig[i]) #Якщо кількість елементів в і-ій групі зi значеннями "одиниця" в першому стовпчику дорівнює нулю, то дія не виконується
...
IG=MainEntropy
for i in range(ngroup):
    IG=IG-Entropies[i]*nig[i]/elem_count
